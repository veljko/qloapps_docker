...
RUN apt-get update \
    ...
    && apt-get install -y git nano vim curl openssh-server unzip \
##setup non root user
    && useradd -m -s /bin/bash ${user} \
    && mkdir -p /home/${user}/www \
##Download Qloapps latest version
    && cd /home/${user}/www && wget https://github.com/Qloapps/QloApps/releases/download/v1.6.0/hotelcommerce.zip && unzip hotelcommerce.zip \
